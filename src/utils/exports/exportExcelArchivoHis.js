...
export async function exportExcelArchivoHis(
  alertasFiltradas,
  tabActiva,
  ...
  const esTabulado = tabActiva === "TABULADO";

  // Nombres de columnas según tab activo (el ancho se autoajusta más abajo)
  const colConfig = esTabulado
    ? [
        { header: "Código" },
        { header: "Ciudadano" },
        { header: "Fecha" },
    ...
    : [
        { header: "Código" },
        { header: "Ciudadano" },
        { header: "Fecha desestimo" },
        { header: "Motivo" },
        { header: "Operador que desestimó" },
      ];

  const headers = colConfig.map(c => c.header);

  const titulo = esTabulado
    ? "REPORTE DE ALERTAS TABULADAS"
  ...
  const filename = esTabulado
    ? `archivo_historico_tabulado_${Date.now()}.xlsx`
    : `archivo_historico_desestimado_${Date.now()}.xlsx`;
