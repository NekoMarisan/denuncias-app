...
import { AlertaCardSkeleton, PanelSkeleton } from "../components/ui/Skeleton";
...
const CentroDespacho = () => {
  ...
  const [cargandoPatrulleros, setCargandoPatrulleros] = useState(true);
  ...
  const panAndZoomSmoothly = useCallback(
    (targetLat, targetLng, targetZoom = TARGET_ZOOM) => {
      if (!map) return;
  ...
  const cargarRutaPatrullero = async (idPatrullero, alerta) => {
    ...
    if (!coordsPat) {
      showToast(
        `El patrullero ${pat.placa || idPatrullero} no tiene ubicación válida`,
        "error",
      );
      return;
    }
    try {
      const routeData = await getRoute(
        [coordsPat.lng, coordsPat.lat],
        [alerta.lng, alerta.lat],
      );
  ...
  `,
      ...
      for (const item of alertas) {
        ...
        const alertaObj = {
          ...
          clasificacionHecho:
            item.contravenciones || item.delitos || "Sin clasificación",
          tipoClasificacion: item.contravenciones
            ? "Contravención"
            : item.delitos
              ? "Delito"
              : null,
  ...
  const cargarPatrulleros = async (esInicial = false) => {
    try {
      const { data, error } = await supabase.from("patrullero").select(`
    id_patrullero,
    placa,
    ubicacion_actual,
  ...
  `);
      ...
      let lista = (data || []).map((p) => ({
        ...
        estado_disponibilidad:
          p.estado_patrullero?.nombre_estado || "DESCONOCIDO",
      ...
      if (esInicial) setCargandoPatrulleros(false);
    }
  };
  const cargarReporte = async (idAlerta) => {
    const { data: reporte } = await supabase
      .from("reporte_alerta")
      .select(
        "id_reporte, id_alerta, id_patrullero, descripcion_reporte, fecha_reporte, resultado",
      )
      .eq("id_alerta", idAlerta)
  ...
  useEffect(() => {
    ...
    Object.values(patrolHalosRef.current).forEach(({ halo }) =>
      halo?.setMap?.(null),
    );
    ...
    const canal = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          const seLibero =
            payload.old?.bloqueado_por && !payload.new?.bloqueado_por;

          if (idAlertaCambio) {
            const nuevoBloqueadoPor = payload.new?.bloqueado_por || null;

            const actualizarBloqueo = (lista) =>
              lista.map((a) =>
                a.id === idAlertaCambio
                  ? {
                      ...a,
                      ...
                      bloqueadoNombre:
                        nuevoBloqueadoPor === a.bloqueadoPor
                          ? a.bloqueadoNombre
                          : null,
                    }
                  : a,
            ...
            if (nuevoBloqueadoPor && nuevoBloqueadoPor !== idOficial) {
              const { data: oficialData } = await supabase
                ...
                .maybeSingle();
              const nombreReal =
                oficialData?.nombre_completo || "Otro despachador";
              const asignarNombre = (lista) =>
                lista.map((a) =>
                  a.id === idAlertaCambio
                    ? { ...a, bloqueadoNombre: nombreReal }
                    : a,
                );
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "patrullero" },
        () => cargarPatrulleros(),
      )
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "patrullero" },
        () => cargarPatrulleros(),
      )
      .on(
        "postgres_changes",
        { event: "DELETE", schema: "public", table: "patrullero" },
        (payload) => {
          const idEliminado = payload.old?.id_patrullero;
          if (idEliminado) {
            setPatrulleros((prev) =>
              prev.filter((p) => p.id_patrullero !== idEliminado),
            );
          }
          cargarPatrulleros();
        },
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "oficial" },
        () => cargarPatrulleros(),
      )
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "oficial" },
        () => cargarPatrulleros(),
      )
      .on(
        "postgres_changes",
        { event: "DELETE", schema: "public", table: "oficial" },
        () => cargarPatrulleros(),
      )
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "evidencia" },
        (payload) => {
          const idAlerta = payload.new?.id_alerta;
          if (idAlerta && idAlerta === intervencionSeleccionadaRef.current) {
            setAlertasIntervencion((prev) =>
              prev.map((a) => {
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "reporte_alerta" },
        (payload) => {
          const idAlerta = payload.new?.id_alerta;
          if (idAlerta && idAlerta === intervencionSeleccionadaRef.current) {
  ...
  useEffect(() => {
    ...
    const haySeleccion =
      ...
      (tabActiva !== "nuevas" && intervencionSeleccionada);
  ...
  useEffect(() => {
    if (map && routeGeometry?.geometry?.coordinates?.length) {
      const bounds = new window.google.maps.LatLngBounds();
      routeGeometry.geometry.coordinates.forEach((c) =>
  ...
  useEffect(() => {
    const intervaloPatrulleros = setInterval(() => {
      cargarPatrulleros();
    }, 15000);
    return () => clearInterval(intervaloPatrulleros);
  }, []);
  ...
  const seleccionarAlerta = useCallback(
    async (alertaId, tipoTab) => {
      const esNuevas = tipoTab === "nuevas";
      const selActual = esNuevas
        ? alertaSeleccionada
        : intervencionSeleccionada;
      ...
      try {
        ...
        const { data: alertaActualDb } = await supabase
          ...
          .maybeSingle();
        if (
          alertaActualDb?.bloqueado_por &&
          alertaActualDb.bloqueado_por !== idOficial
        ) {
          showToast(
            "Esta alerta ya está siendo gestionada por otro despachador",
            "error",
          );
          return;
        }
        ...
        const alerta = [...alertasNuevas, ...alertasIntervencion].find(
          (a) => a.id === alertaId,
        );
        if (!alerta) return;
        ...
        if (clasificacion) {
          const update = (lista) =>
            lista.map((a) =>
              a.id === alertaId
                ? { ...a, clasificacionHecho: clasificacion }
                : a,
            );
        ...
        setTimeout(
          () => iniciarPing(alertaId, alerta.lat, alerta.lng, color),
          600,
        );
      } catch (err) {
  ...
  const despacharPatrullero = (idPatrullero) => {
    ...
    if (pendingAsignaciones[idPatrullero] === alertaSeleccionada) {
      ...
      setPendingAsignaciones((prev) => ({
        ...prev,
        [idPatrullero]: alertaSeleccionada,
      }));
    }
  };
  ...
  const finalizarAsignacion = async () => {
    if (!alertaSeleccionada) return;
    const pendientes = Object.keys(pendingAsignaciones).filter(
      (k) => pendingAsignaciones[k] === alertaSeleccionada,
    );
  ...
  const enviarATabulacion = async () => {
    ...
    if (!alertaObj?.reportePatrullero) {
      showToast(
        "Aún no se ha recibido el reporte policial. No es posible continuar.",
        "error",
      );
      return;
    }
  ...
  const finalizarCaso = async (tipoCierre) => {
    ...
    showToast(
      `Caso finalizado: ${tipoCierre === "derivar" ? `DERIVADO A ${derivacionDestino}` : "ATENDIDO"}`,
      "success",
    );
  ...
  const alertasVisibles =
    tabActiva === "nuevas" ? alertasNuevas : alertasIntervencion;
  const alertaActual =
    tabActiva === "nuevas"
  ...
  if (loadError)
    return <div className="p-4 text-red-600">Error cargando Google Maps</div>;

  return (
    <div className="h-full w-full max-w-full overflow-x-hidden overflow-y-scroll [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden sm:overflow-y-auto sm:[scrollbar-width:auto] sm:[-ms-overflow-style:auto] sm:[&::-webkit-scrollbar]:block font-sans">
     <div className="-mt-6 w-full max-w-full box-border px-1.5 sm:px-1 pb-4">
        <div className="flex flex-col lg:flex-row gap-6 items-stretch lg:items-start w-full max-w-full box-border">
          {/* SIDEBAR */}
 <div className="mt-5 w-full max-w-full box-border md:max-h-[88vh] md:overflow-hidden rounded-2xl bg-white shadow-md lg:sticky lg:-top-6 lg:h-[calc(100vh-2rem)] lg:w-[280px] xl:w-[320px] 2xl:w-[360px] lg:shrink-0">
            <div className="flex md:h-full flex-col">
              <div className="p-3 bg-white border-b border-slate-100 rounded-t-2xl">
                <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-200">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center py-3 rounded-lg font-medium text-[12px] uppercase transition-all tracking-widest ${tabActiva === "nuevas" ? "bg-[#474b29] text-white shadow-md" : "text-slate-400 hover:bg-white"}`}
                  >
                    REVISADAS
                  </button>
                  <button
                    onClick={() => setTabActiva("intervencion")}
                    className={`flex-1 flex items-center justify-center py-3 rounded-lg font-medium text-[12px] uppercase transition-all tracking-widest ${tabActiva === "intervencion" ? "bg-[#474b29] text-white shadow-md" : "text-slate-400 hover:bg-white"}`}
                  >
                    EN INTERVENCIÓN
                  </button>
                </div>
              ...
              <div className="md:flex-1 md:overflow-hidden flex flex-col">
                {cargando ? (
                  <>
                    {/* Emergencias — skeleton */}
                    <div className="flex flex-col py-4 flex-1 min-h-0">
                      ...
                      <div className="overflow-hidden min-h-0 px-3 space-y-3.5 flex-1">
                        {[0, 1, 2].map((i) => (
                  ...
                  <>
                    {/* Emergencias */}
                    <div className="flex flex-col py-4 md:flex-1 md:min-h-0">
                      <h3 className="text-[11px] font-medium text-slate-400 uppercase mb-3 flex items-center gap-1 tracking-wider px-3 shrink-0">
                        <span className="w-1.5 h-1.5 bg-[#C90A0A] rounded-full animate-pulse" />
                        Alertas de Emergencia
                      </h3>
                      <div className="overflow-y-auto min-h-0 px-3 space-y-3.5 scroll-hover max-h-[210px] sm:max-h-[220px] md:max-h-none md:flex-1">
                        {alertasVisibles
                          ...
                          .map((a) => (
                            <AlertaCard
                              ...
                              bloqueadaPorOtro={
                                a.bloqueadoPor &&
                                a.bloqueadoPor !== user?.id_oficial
                              }
                              bloqueadoNombre={a.bloqueadoNombre}
                            />
                          ))}
                        {alertasVisibles.filter((a) => a.tipo === "EMERGENCIA")
                          .length === 0 && (
                          <p className="text-[11px] text-slate-400 font-medium tracking-wider text-center py-3">
                            Sin emergencias
                          </p>
                        )}
                      </div>
                    ...
                    <div className="flex flex-col py-4 md:flex-1 md:min-h-0">
                      ...
                      <div className="overflow-y-auto min-h-0 px-3 space-y-3.5 scroll-hover max-h-[210px] sm:max-h-[220px] md:max-h-none md:flex-1">
                        ...
                        {alertasVisibles.filter((a) => a.tipo === "CIUDADANA")
                          .length === 0 && (
                          <p className="text-[11px] text-slate-400 font-medium text-center py-3 tracking-wider">
                            Sin alertas ciudadanas
                          </p>
                        )}
                      </div>
                    </div>
          ...
          <div
            ref={panelCentralRef}
            className="mt-5 flex-1 min-h-0 flex flex-col gap-3 sm:gap-4 lg:px-1 lg:pb-2 lg:sticky lg:-top-6 lg:h-[calc(100vh-6.7rem)]"
          >
            <div
              className={`w-full shrink-0 overflow-hidden rounded-2xl bg-white shadow-md transition-all duration-300 aspect-[4/3] sm:aspect-[16/9] md:aspect-[16/9] ${
                tabActiva === "intervencion"
                  ? "lg:aspect-auto lg:h-[324px]"
                  : "lg:aspect-auto lg:h-[400px]"
              }`}
            >
              {!isLoaded ? (
                <PanelSkeleton heightClass="h-full" />
              ) : (
                <GoogleMap
                ...
                >
                  ...
                  {routeGeometry?.geometry && (
                    <Polyline
                      path={routeGeometry.geometry.coordinates.map((c) => ({
                        lat: c[1],
                        lng: c[0],
                      }))}
                      options={{
                        strokeColor: COLOR_CIUDADANA,
                        strokeWeight: 5,
                        strokeOpacity: 0.8,
                      }}
                    />
            ...
            <div
              className={`flex gap-2 sm:gap-3 md:gap-4 flex-wrap text-[8px] sm:text-[9px] md:text-[9.5px] font-medium uppercase tracking-widest text-slate-500 ${
                tabActiva === "intervencion" ? "hidden" : ""
              }`}
            >
              {[
            ...
            <div
              ...
              className={`flex-1 min-h-0 flex flex-col overflow-y-auto pb-1 transition-[padding] duration-[2500ms] ease-in-out ${
                tabActiva === "intervencion" ? "gap-2" : "gap-4"
              } ${scrollActivo ? "pr-2 scroll-visible" : "pr-0 scroll-hover"}`}
            >
              <DetalleAlerta
                ref={detalleRef}
                alerta={alertaActual}
              ...
              <GestionPatrullas
                ...
                cargandoPatrulleros={cargandoPatrulleros}
