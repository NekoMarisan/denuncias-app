...
export const GestionPatrullas = ({
  ...
  const [hoverPatrulleros, setHoverPatrulleros] = useState(false);
    ...
    const cargar = async () => {
      ...
      if (!error && data) setInstituciones(data);
        ...
        setInstituciones([
          {
            id: 1,
            nombre: "FELCC (FUERZA ESPECIAL DE LUCHA CONTRA EL CRIMEN)",
          },
          {
            id: 2,
            nombre: "FELCV (FUERZA ESPECIAL DE LUCHA CONTRA LA VIOLENCIA)",
          },
          {
            id: 3,
            nombre: "FELCN (FUERZA ESPECIAL DE LUCHA CONTRA EL NARCOTRÁFICO)",
          },
  ...
  const puedeDerivarse = patrullasAsignadas.some(
    (p) => p.id_estado === ESTADO_REVISION,
  );
  ...
  const handleEnviarTabulacion = async () => {
    ...
    if (!alertaActual?.reportePatrullero) {
      showToast(
        "Aún no se ha recibido el reporte policial. No es posible enviar a tabulación.",
        "error",
      );
      return;
    }
    ...
    if (!Derivacion) {
      showToast(
        "Debe seleccionar una opción de derivación (o 'Sin derivación') antes de continuar.",
        "error",
      );
      return;
    }
  ...
  const renderEvidencia = (ev, idx) => {
    ...
    if (esVideo) {
      return (
        <div
          key={idx}
          className="h-full border border-slate-200 shadow-md rounded-lg p-0.5 bg-white"
        >
          {preview}
        </div>
      );
    ...
    return (
      <div
        key={idx}
        className="h-full border border-slate-200 shadow-md rounded-lg p-0.5 bg-white group relative"
      >
        <a
          href={ev.archivo}
          target="_blank"
          rel="noreferrer"
          className="block h-full overflow-hidden rounded-lg cursor-pointer"
        >
          {preview}
        </a>
      </div>
  ...
  const getEstadoInfo = (p) => {
    if (!p.activo)
      return {
        texto: "NO CONECTADO",
        claseBtn: "bg-gray-300 text-gray-500 cursor-not-allowed",
    ...
    switch (p.id_estado) {
        ...
        return {
          texto:
            pendingAsignaciones[p.id_patrullero] === alertaSeleccionada
              ? "SELECCIONADA"
              : "DESPACHAR",
          claseBtn: alertaSeleccionada
            ? " text-xs bg-[#474b29] hover:bg-[#3a3e21] text-white tracking-widest"
            : "text-xs bg-slate-300 text-white cursor-not-allowed tracking-widest",
          borde: "border-[#474b29]",
      ...
      case ESTADO_NOTIFICADO:
        return {
          texto: "NOTIFICADO",
          claseBtn: "bg-blue-600 text-white cursor-not-allowed",
      ...
      case ESTADO_EN_CAMINO:
        return {
          texto: "EN CAMINO",
          claseBtn: "bg-orange-600 text-white cursor-not-allowed",
      ...
      case ESTADO_EN_LUGAR:
        return {
          texto: "EN EL LUGAR",
          claseBtn: "bg-purple-700 text-white cursor-not-allowed",
      ...
      case ESTADO_REVISION:
        return {
          texto: "EN REVISIÓN",
          claseBtn: "bg-gray-600 text-white cursor-not-allowed",
      ...
      default:
        return {
          texto: "NO DISPONIBLE",
          claseBtn: "bg-slate-300 text-white cursor-not-allowed",
  ...
  return (
    <div
      className={`bg-white p-3 xs:p-4 sm:p-4 rounded-2xl shadow-md border border-slate-200 relative z-0 flex flex-col w-full shrink-0 ${
        isNuevas
          ? "h-[45vh] xs:h-[42vh] sm:h-[38vh] md:h-[36vh] lg:h-[420px]"
          : "h-[55vh] sm:h-[48vh] md:h-[44vh] lg:h-[519px] mt-3"
      }`}
    >
      <div className="flex flex-wrap justify-between items-center gap-2 mb-4">
        <h3
          className="font-bold uppercase flex items-center gap-2 tracking-wider text-[#1e293b] 
      ...
      {isNuevas ? (
        <>
          <div className="relative -mt-2 flex-1 min-h-0">
            <div
  onMouseEnter={() => setHoverPatrulleros(true)}
              onMouseLeave={() => setHoverPatrulleros(false)}
               className="absolute inset-0 overflow-y-scroll pr-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden sm:scroll-hover sm:[scrollbar-width:auto] sm:[-ms-overflow-style:auto] sm:[&::-webkit-scrollbar]:block grid grid-cols-1 gap-2 px-0.5 w-full xs:grid-cols-1 xs:gap-2 sm:grid-cols-2 sm:gap-2.5 sm:px-1 md:grid-cols-2 md:gap-3.5 lg:grid-cols-2 lg:gap-3.5 xl:grid-cols-2 xl:gap-4 2xl:grid-cols-3 2xl:gap-4"
            >
              {cargandoPatrulleros
                ? [0, 1, 2, 3, 4, 5, 6, 7].map((i) => (
                    <PatrulleroCardSkeleton key={i} />
                  ))
                : patrulleros.map((p) => {
                    const pendiente =
                      alertaSeleccionada &&
                      pendingAsignaciones[p.id_patrullero] ===
                        alertaSeleccionada;
                    const asignada =
                    ...
                    return (
                      <div
                        key={p.id_patrullero}
                        className={`h-[108px] p-2.5 xs:h-[112px] xs:p-2.5 sm:h-[92px] sm:p-2.5 md:h-[130px] md:p-3 rounded-lg border shadow-md transition-all flex flex-col justify-between ${bordeClase} ${
                          resaltar
                            ? "bg-blue-50/30"
                            : fueraServicio
                      ...
                      >
                        <div>
                          <div className="flex justify-between items-start gap-1">
                            <span className="text-[12px] font-medium tracking-wider text-slate-500 uppercase truncate">
                          ...
                          <p className="text-[11.5px] font-medium text-[#1e293b] leading-tight truncate mt-1.5 md:mt-3 tracking-wider">
                            {p.nombre_oficial}
                          </p>
                          <p
                        ...
                        <button
                          ...
                          className={`w-full py-1.5 md:py-2 px-1 text-[9.5px] xs:text-[10px] sm:text-[11px] font-medium tracking-normal sm:tracking-wider rounded-md uppercase transition-all shadow active:scale-95 whitespace-nowrap overflow-hidden text-ellipsis ${estadoInfo.claseBtn}`}
                        >
                          {estadoInfo.texto}
                        </button>
                      </div>
            ...
            <div
              className={`absolute top-0 right-0 bottom-0 w-3.5 bg-white pointer-events-none transition-opacity duration-150 ${
                hoverPatrulleros ? "opacity-0" : "opacity-100"
              }`}
            />
          </div>
        ...
        <div className="flex-1 min-h-0 overflow-hidden pr-1 w-full">
          <div className="flex flex-col lg:flex-row items-stretch gap-4 w-full min-w-0 h-full">
                    ...
                    {patrullasAsignadas.map((p) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p
                            className={`text-[10px] font-medium tracking-wider uppercase mt-1 truncate ${estadoColor}`}
                          >
                            {badge}
                          </p>
                        </div>
              ...
              <div className="-mt-2">
                <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider flex items-center gap-1 mb-2">
                  ...
                  {alertaActual?.reportePatrullero &&
                    alertaActual?.idPatrulleroReporte && (
                      <span className="normal-case text-slate-400">
                        ...
                        {patrulleros.find(
                          (p) =>
                            p.id_patrullero ===
                            alertaActual.idPatrulleroReporte,
                        )?.placa || alertaActual.idPatrulleroReporte}
                      </span>
                    )}
                </p>
                <div className="p-3 bg-white rounded-xl border border-slate-200 min-h-[100px] shadow-sm overflow-y-auto scroll-hover">
                  {alertaActual?.reportePatrullero ? (
              ...
              <div className="space-y-2 mt-1">
                ...
                <div className="relative">
                  <select
                    ...
                    className={`w-full h-11 pl-3 pr-7 border border-slate-200 rounded-xl text-[12px] outline-none appearance-none transition-colors font-medium tracking-wider ${
                      puedeDerivarse
                        ? "text-slate-500 focus:border-[#474b29]"
                        : "text-slate-400 bg-slate-100 cursor-not-allowed"
                    }`}
                    title={
                      !puedeDerivarse
                        ? "Solo disponible cuando la unidad esté EN REVISIÓN"
                        : undefined
                    }
                  >
                    <option value="">
                      {puedeDerivarse
                        ...
                        : "Esperando que la unidad esté en revisión"}
                    </option>
            ...
            <div
              ...
              className="w-full lg:w-56 xl:w-64 2xl:w-72 shrink-0 flex flex-col min-w-0"
            >
              <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1">
                Evidencias
                <span>
                  ({alertaActual?.reportePatrullero ? evidencias.length : 0})
                </span>
              </p>
              {!alertaActual?.reportePatrullero ? (
                ...
                <div
                  className={`flex-1 min-h-[240px] max-h-[400px] overflow-y-auto pr-1 grid grid-rows-3 gap-2 ${
                    hoverEvidencias ? "scroll-visible" : "scroll-hover"
                  }`}
                >
                  {evidencias
                    .slice(0, 3)
                    .map((ev, idx) => renderEvidencia(ev, idx))}
                </div>
              ) : (
                <div className="flex-1 min-h-[200px] flex items-center justify-center bg-slate-50 rounded-md border border-slate-200 border-dashed">
          ...
          <div className="flex flex-col lg:flex-row items-stretch gap-4 h-full">
            <div className="flex-1 min-w-0 flex flex-col gap-4">
              <div>
                ...
                <p className="text-[11px] text-slate-400 font-medium tracking-wider min-h-[6svh]">
                  Sin unidades asignadas
                </p>
              </div>
              <div className="border-t border-slate-100 my-4 mt-4"></div>
              <div className="-mt-2.5">
                <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider flex items-center gap-1 mb-2">
                  Reporte del patrullero
                </p>
                <div className="p-3 bg-slate-50 rounded-md border border-slate-200 min-h-[110px] shadow-sm">
                  <p className="text-[11px] text-slate-400 font-medium tracking-wider">
                    Sin reporte enviado aún.
                  </p>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <select
                    disabled
                    className="w-full font-medium border border-slate-200 rounded-xl shadow-sm bg-white text-slate-400 outline-none cursor-not-allowed appearance-none
                ...
                <button
                  disabled
                  className="!mt-5 w-full h-11 py-2.5 rounded-xl font-medium text-[11.5px] uppercase tracking-wider border border-slate-300 text-slate-500 bg-slate-200 flex items-center justify-center gap-2 cursor-not-allowed"
                >
                  ESPERANDO REPORTE DEL PATRULLERO
                </button>
              </div>
            ...
            <div className="w-full lg:w-56 xl:w-64 2xl:w-72 shrink-0 flex flex-col overflow-hidden">
              <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider mb-2">
                Evidencias <span>(0)</span>
              </p>
